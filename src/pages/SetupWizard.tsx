...
export default function SetupWizard({ open, onClose }: SetupWizardProps) {
  ...
  // (src/components/config/WeGameInstaller.tsx).
  ...
  // --- Step 5 handled by <WeGameInstaller variant="wizard"> -------------
  // All install/reinstall/progress logic lives in that shared component.
      ...
      <div className="glass-card p-6 animate-fade-in min-h-[320px]">
          ...
          <div className="space-y-5">
                    ...
                    <div key={dep.id} className={`rounded-lg border overflow-hidden ${
                      ...
                      <div className="px-4 py-3 space-y-2">
                        ...
                        {/* PRD v1.8: for winetricks, offer a one-click
                            ...
                            ("一键下载到 ~/.local/bin"). */}
                        {state.selection === "download" && dep.id === "winetricks" && (
                          <div className="ml-6">
                            <button
                              onClick={async () => {
                                try {
                                  const r = await invoke<{ success: boolean; path?: string; error?: string }>(
                                    "install_winetricks_userlocal",
                                  );
                                  if (r.success) {
                                    // Re-run the scan so this dep flips to "scanned" automatically.
                                    await runScan();
                                  } else {
                            ...
                            >
                              ...
                              立即下载到 ~/.local/bin（无需密码）
                            </button>
                            <p className="mt-1 text-[10px] text-gray-500">
                              推荐：下载后自动重新扫描，此依赖即标记为就绪。
                            </p>
                          </div>
                        )}
                      </div>
            ...
            <div className="space-y-4 pt-4 border-t border-white/10">
              ...
              <ProtonPicker
                ...
                variant="wizard"
                onSelect={(path) => updateConfig({ proton_path: path })}
                hideHeader={true}
              />
              <button
                onClick={() => {
        ...
        {currentStep === 3 && (
          <div className="space-y-5">
            <div className="mb-4">
              ...
              <p className="mt-1 text-sm text-gray-400">配置 Wine 前缀与 WeGame 本体的安装路径。</p>
            </div>
            ...
            <PathsSection
              ...
              saveEnvironment={saveEnvironment}
              variant="wizard"
              onLocalChange={(partial) => updateConfig(partial)}
            />
        ...
        {currentStep === 5 && (
          <WeGameInstaller
            config={localConfig}
            variant="wizard"
            onStatusChange={({ installed }) => setWegameInstalled(installed)}
          />
        )}
      </div>
