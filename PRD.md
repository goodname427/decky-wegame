...
- **未安装**：展示黄色提示卡 + 三按钮（v1.8.1 调整）：
  - 主推「选择本地安装器文件」（调用 `pick_wegame_installer` 原生文件选择器 → `install_wegame_from_local`）
  - 次级「打开官网」（`window.open("https://www.wegame.com.cn/")` 在系统浏览器打开）
  - 粘底「在线下载（实验）」（调用 `install_wegame`，依次尝试候选源 URL，源变动时则用户有验证方式）
- **安装流程（v1.8.1 修正）**：腾讯已不再提供 WeGame 安装器的稳定公网直链（原 `dldir1.qq.com/WeGame/Setup/WeGameSetup.exe` 返回 404），因此采用「本地文件优先」策略：
  1. **本地文件路径**（推荐）：`pick_wegame_installer` 弹出原生文件选择器，用户选中浏览器里下好的 `WeGameSetup.exe`；后端 `installWegameFromLocalFile` 校验文件（存在 / 非空目录 / 大小 ≥ 1 MB / 扩展名 `.exe`）、复制到缓存 `~/.cache/decky-wegame/installers/WeGameSetup.exe`（后续重装无需再选）、运行在前缀内。
...
- **失败恢复（v1.8.1）**：错误卡首先引导用户「选本地文件」，同时保留「打开官网」、「重试在线下载」、「清缓存并重新下载」四个动作。
...
| `pick_wegame_installer`（**v1.8.1 新增**） | Electron 原生文件选择器，过滤器 `*.exe`，返回 `{ canceled, filePath }` |
| `install_wegame_from_local`（**v1.8.1 新增**） | 从用户选定的本地 `.exe` 安装（主路径） |
...
| 日期 | 版本 | 变更说明 |
|------|------|---------|
| 2026-04-18 | v1.8.1 | **修复 WeGame 安装器下载 404**：腾讯 CDN 已不再提供 `https://dldir1.qq.com/WeGame/Setup/WeGameSetup.exe`（探测了 10+ 个常见路径变体全部 404）。采纳 A+C 方案：默认「本地文件优先」仍保留「在线下载」作实验性兜底。`wegame_installer.ts` 新增 `DEFAULT_WEGAME_INSTALLER_URL_CANDIDATES` 候选列表 + `probeUrl` HEAD 预探测，表头优先度为用户 `extra_env_vars.WEGAME_INSTALLER_URL`；新增 `installWegameFromLocalFile`（校验 .exe、复制到缓存、运行于 prefix）。IPC 新增 2 个：`pick_wegame_installer`（Electron 原生文件选择器）、`install_wegame_from_local`。`<WeGameInstaller>` 在 wizard / manage 两种形态下均提供三路入口：主推「选本地安装器文件」（neon-primary）、次级「打开官网」（https://www.wegame.com.cn/ 在系统浏览器打开）、粘底「在线下载（实验）」；失败时错误面板会主动引导用户回本地文件路径。不内置第三方镜像源（版权考量） |
