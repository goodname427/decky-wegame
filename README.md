...
> 当前版本：**v1.10.0** · 需求定义见 [PRD.md](./PRD.md)，关键开发记录见 [DEVLOG.md](./DEVLOG.md)

## 功能特性

- **一键自动安装** - 首启弹出欢迎页，点下大号「🚀 一键自动安装」按钮后依次自动完成 Proton 选择 / Wine Prefix 创建 / 核心依赖预拉取 / WeGame 本体安装；全程不需要用户手动配置代理、找镜像或搜资源包
- **高级模式逃生舱** - 欢迎页右下角小按钮「高级模式 →」可随时跳过自动流程直接进入完整的 5 步向导；自动配置进行中也支持「切换到高级模式」中途逃生，已完成的状态会预填到高级模式中
- **多镜像池内置** - 所有境外资源（Proton-GE、winetricks 依赖、WeGame 安装器等）均内置国内镜像池 + GitHub 加速反代 + 本地文件逃生舱 三层降级，任何一层失效自动倒- **5 步安装向导（高级模式）** - 从「中间层检测」到「WeGame 本体安装」的全流程细粒度设置
