...
> 当前版本：**v1.12.0** · 需求定义见 [PRD.md](./PRD.md)，关键开发记录见 [DEVLOG.md](./DEVLOG.md)

## 功能特性

- **一键自动安装** - 首启弹出欢迎页，点下大号「🚀 一键自动安装」按钮后依次自动完成 Proton 选择 / Wine Prefix 创建 / 核心依赖预拉取 / WeGame 本体安装；全程不需要用户手动配置代理、找镜像或搜资源包
- **高级模式逃生舱** - 高级模式的合法入口有三处：欢迎页右下角「高级模式 →」小按钮（首启可见）、自动配置进度页右上角「切换到高级模式 →」（运行中逃生，已完成状态会预填到向导里）、依赖管理页顶部「重新配置环境」（事后再调整）
