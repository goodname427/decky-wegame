...
> 当前版本：**v1.8.1**（2026-04-18）· 详见 [PRD.md](./PRD.md) 与 [DEVLOG.md](./DEVLOG.md)
...
- **5 步安装向导** - 引导用户完成从"中间层检测"到"WeGame 本体安装"的全流程
  ...
  5. 安装 WeGame（**首推用户从 [wegame.com.cn](https://www.wegame.com.cn/) 下载官方 `WeGameSetup.exe` 后选本地文件导入**；保留在线下载作实验性兜底）
...
- **WeGame 本体管理（v1.8.1 调整）** - 腾讯已不再提供稳定的 WeGame 安装器直链，因此改为「本地文件优先」策略：向导 / 依赖管理页均提供「选择本地安装器文件」（主推）、「打开官网」、「在线下载（实验）」三类入口；在线下载会按候选 URL 列表依次探测，全部失败时自动引导用户回到本地文件导入
- **WeGame 本体管理** - 依赖管理页新增 WeGame 安装状态卡片，支持直接「用本地文件重装」或「清缓存并在线重装」，无需重新进入向导
...
6. **步骤 5** 安装 WeGame：腾讯已不再提供 WeGame 安装器的稳定公网直链，**推荐先在浏览器里打开 [www.wegame.com.cn](https://www.wegame.com.cn/) 下载 `WeGameSetup.exe`，再回到本应用点「选择本地安装器文件」导入**；向导中同时提供「打开官网」一键跳转与「在线下载（实验）」兜底，实验性选项会依次尝试内置候选源，全部失败时引导您回到本地文件方式
